...
export class AppComponent implements OnInit {
  ...
  beautify = () => {
    if (this.selectedLanguageType === 'sql') {
      let tempCode: string = null;
      if (this.sqlOptions.upperCase) {
        tempCode = this.toUpperCaseQuoted(this.code);
      }
      const indent = '\xa0';
      this.sqlOptions.indent = indent.repeat(this.sqlOptions.indentation);
      tempCode = tempCode || this.code;
      this.previewCode = this.global.sqlFormatter(tempCode, this.sqlOptions);
    } else if (this.selectedLanguageType === 'json') {
      this.previewCode = this.global.beautify.js(this.code, this.options);
    } else {
      this.previewCode = this.global.beautify[this.selectedLanguageType](this.code, this.options);
    }
    this.setLocalStorage(null, null);
  }
  ...
  setOptions = () => {
    ...
    this.selectedLanguageType = 'json';
    this.options = {
      indent_size: 4,
      indent_char: ' ',
      eol: '\n',
